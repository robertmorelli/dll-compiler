...
public partial class Evaluator
{
    ...
    private static readonly TokenProcFunc nothingFunc = token => { };

    private static readonly TokenProcFunc everythingFunc = token =>
    {
        throw new ArgumentException(string.Format("Could not identify token ({0})", token));
    };

    private static readonly KeyValuePair<Regex, TokenProcFunc>
        errorTokenAction = new(isAnythingRegex(), everythingFunc);
        ...
        Lookup variableEvaluator)
    {
        ...
        if (valueStack.Count != 1) throw new ArgumentException("Too many or too few values for amount of operators");
    ...
    /// </exception>
    private static Dictionary<Regex, TokenProcFunc> generateTokenProcessorDictionary(Stack<int> valueStack,
        Stack<string> operatorStack, Lookup variableEvaluator)
    {
        /// <summary>
        ...
        /// </summary>
        void multiplicativefunc(string token)
        {
            operatorStack.Push(token);
        }
        ...
        /// </summary>
        void openParenFunc(string token)
        {
            operatorStack.Push(token);
        }
    ...
    [GeneratedRegex("\\s*(\\(|\\)|-|\\+|\\*|/)\\s*")]
    private static partial Regex findTokenRegex();

    [GeneratedRegex("^(\\s*)$")]
    private static partial Regex isWhiteSpaceRegex();

    [GeneratedRegex("^(\\d+)$")]
    private static partial Regex isIntRegex();

    [GeneratedRegex("^(\\w+\\d+)$")]
    private static partial Regex isVariableRegex();

    [GeneratedRegex("^(\\+|-$)")]
    private static partial Regex isAddativeRegex();

    [GeneratedRegex("^(\\+)$")]
    private static partial Regex isAddRegex();

    [GeneratedRegex("^(\\*|/)$")]
    private static partial Regex isMultiplicativeRegex();

    [GeneratedRegex("^(\\*)$")]
    private static partial Regex isMultRegex();

    [GeneratedRegex("^(\\()$")]
    private static partial Regex isOpeningParenRegex();

    [GeneratedRegex("^(\\))$")]
    private static partial Regex isClosingParenRegex();

    [GeneratedRegex("^(.*)$")]
    private static partial Regex isAnythingRegex();

    [GeneratedRegex("^(0+)$")]
    private static partial Regex isZeros();
