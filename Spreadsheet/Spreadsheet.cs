...
internal partial class Utility
{
    ...
    [GeneratedRegex(@"^[a-zA-Z][a-zA-Z\d]*$", RegexOptions.IgnorePatternWhitespace |
...
public class Spreadsheet : AbstractSpreadsheet
{
                    ...
                    while (reader.Read())
                        switch (reader.NodeType)
                        {
                            case XmlNodeType.EndElement:
                                if (reader.Name == "cell")
                                    goto
                                        exitloop; //AAAAAH CALL THE POILCE ITS A GOTO (no break loop in switch and no labeled break)
                                break;
    ...
    /// <returns></returns>
    protected new IList<string> GetCellsToRecalculate(string start)
    {
    ...
    protected override IList<string> SetCellContents(string name, string text)
    {
        ...
        var deps = GetCellsToRecalculate(name);

        cells.Remove(name);
        //store only if its not empty
        ICell newCell = new StringCell(text);
        if (!(cells.TryGetValue(name, out var oldCell) && newCell == oldCell))
        {
            cells[name] = newCell;
    ...
    public override string GetXML()
    {
        ...
        DoXMLWriting(XmlWriter.Create(stringWriter, new XmlWriterSettings { Indent = true, IndentChars = "  " }));
    ...
    protected struct FormulaCell(Formula f, Spreadsheet s) : ICell
    {
        ...
        public void Compute()
        {
            ...
            foreach (var dep in vars)
                if (!spreadsheet.cells.TryGetValue(dep, out var cell) || cell.Value.GetType() != typeof(double))
                {
    ...
    protected struct RecalculatedCellList(string name, Spreadsheet s) : IList<string>
    {
        ...
        private struct IRecompStackFrame
        {
            public string name;
                    ...
                    if (frame.firstHalf) //ret pops either &Visit or &Visit + K
                    {
                        ...
                        foreach (var dep in _spreadsheet.GetDirectDependents(frame.name))
                            if (!visited.Contains(dep))
                                virtualCallStack.Push(new IRecompStackFrame { name = dep, firstHalf = true });
                    }
                    else
                    {
