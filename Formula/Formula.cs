...
public static partial class Utility
{
    ...
    private const RegexOptions ro =
        RegexOptions.IgnorePatternWhitespace |
        RegexOptions.NonBacktracking;

    [GeneratedRegex(@"^\s*$", ro)]
    public static partial Regex isWhiteSpaceRegex();

    [GeneratedRegex(@"^[a-zA-Z][a-zA-Z\d]*$", ro)]
    public static partial Regex isVariableRegex();

    [GeneratedRegex(@"\b|([\-)*(+/])", ro)]
    public static partial Regex bounderies();
}
...
public class Formula
{
    ...
    private readonly bool ThrowDBZEveryTime;

    private readonly Token[] Tokens;
    private readonly string[] VarTokens;
    private readonly TokenNode ExecutableAst;
    private bool HasHashCode;
    private int HashCodeCash;
    ...
    public Formula(string formula, Func<string, string> normalize, Func<string, bool> isValid)
    {
        ...
        if (valueStack.Count > 0 || operatorStack.Count > 0)
            throw new FormulaFormatException("unmatched parenthesis or other operator");
    ...
    public object Evaluate(Func<string, double> lookup)
    {
        ...
        {
            return new FormulaError(ToString() + " : " + e.Message);
        }
    }
    ...
    /// </summary>
    private static IEnumerable<Token> GetNormalTokens(string formula, Func<string, string> normalize,
        Func<string, bool> isValid)
    {
        ...
        foreach (var s in Utility.bounderies().Split(formula))
            if (!Utility.isWhiteSpaceRegex().IsMatch(s))
                ret.Add(new Token(s, normalize, isValid));
        return ret;
    }
    ...
    /// </exception>
    private static Dictionary<TokenType, TokenProcFunc> generateTokenProcessorDictionary(Stack<TokenNode> valueStack,
        Stack<Token> operatorStack, Func<string, bool> isValid)
    {
        /// <summary>
        ...
        /// </summary>
        void multiplicativefunc(Token token)
        {
            operatorStack.Push(token);
        }
        ...
        void closeParenFunc(Token token)
        {
            ...
            if (operatorStack.Count == 0 || !operatorStack.Pop().isLParens)
                throw new FormulaFormatException("Unmatched closing parenthesis");
            if (operatorStack.Count != 0 && operatorStack.Peek().IsMultiplicative)
            {
    ...
    internal struct TokenNode
    {
        ...
        public TokenNode(Token primary, TokenNode? left, TokenNode? right)
        {
            ...
            _leftRef = new ReferenceLMAO(left);
            _rightRef = new ReferenceLMAO(right);
        ...
        internal class ReferenceLMAO(TokenNode? tn)
        {
            public TokenNode? tn = tn;
        }
        ...
        public readonly TokenNode? LeftChild => _leftRef.tn;
        public readonly TokenNode? RightChild => _rightRef.tn;
        ...
        public readonly bool HasConstValue => !double.IsNaN(constValue);
    ...
    internal struct Token
    {
        ...
        public readonly bool IsAddative => isAdd || isSub;

        public bool isAdd = false;
        public bool isSub = false;
        public readonly bool IsMultiplicative => isMult || isDiv;
        ...
        public Token(string primative, Func<string, string> normalizer, Func<string, bool> isValid)
        {
            ...
            if (isImm = double.TryParse(primative, out var d))
            {
                primativeString = d.ToString();
            }
            else if (isVar)
            {
