...
public static partial class Utility
{
    ...
    [GeneratedRegex(@"^\p{L}[\p{L}\p{Nd}]*$", RegexOptions)]
...
public class Formula
{
    ...
    public object Evaluate(Func<string, double> lookup)
    {
        ...
        try
        {
            return _executableAst.Value(lookup);
        }
        catch (Exception e)
        {
    ...
    public struct TokenNode
    {
        ...
        public readonly TokenNode Optimized(bool unsafeOptimizations = true)
        {
            ...
            if (_primary.isSub)
                // % - 0 = %
                if (realRight._constValue.Equals(0))
                    return realLeft;
            if (_primary.isDiv)
                // % / 1 = %
                if (realRight._constValue.Equals(1))
                    return realLeft;
            ...
            return new TokenNode(_primary, realLeft, realRight);
        }
        ...
        private class ReferenceLmao(TokenNode? tn)
        {
            public readonly TokenNode? tn = tn;
        }
        ...
        private readonly TokenNode? LeftChild => _leftRef.tn;
        private readonly TokenNode? RightChild => _rightRef.tn;
